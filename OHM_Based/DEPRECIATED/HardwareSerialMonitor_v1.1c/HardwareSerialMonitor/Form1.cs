...
namespace HardwareSerialMonitor
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            //thisComputer.FanControllerEnabled = true; // added RSH 30/11/2020  Computer.cs / IComputer.cs / ISensor.cs / Sensors.cs
            //thisComputer.networkEnabled = true;  // taken from libreHardwareMonitor
            thisComputer.Open();
        ...
        void CreateMenuItems()
        {
            ...
            item.Text = "Serial Ports Available";         // set items text to "serial ports"
        ...
        private void dataCheck()//function overload with 0 arguments, called when a with the default serial port on a timer
        {
            ...
            string cpuTemp = "C";
            string cpuLoad = "c";
            string CpuFanSpeedLoad = "";       // Not Working

            //------------------------------------
            
            string ramLoad = "RL";
            string ramUsed = "R";              // LibreHardwareMonitor and OpenHardwareMonitor used different Case definitions, see further down!!!!
            string ramAvailable = "RA";        // This is the RAM still available, not the total!!!
            ...
            string gpuMemTotal = "GMT";        //added RSH 30/11/2020  see NvidiaGPU.cs "memoryAvail = new Sensor("GPU Memory Total", 3, SensorType.SmallData, this, settings);"
            string gpuMemUsed = "GMU";         //added RSH 4/12/2020   see NvidiaGPU.cs  "memoryUsed = new Sensor("GPU Memory Used", 2, SensorType.SmallData, this, settings);"
            string gpuMemLoad = "GML";         //added RSH 4/12/2020   see NvidiaGPU.cs  "memoryLoad = new Sensor("GPU Memory", 4, SensorType.Load, this, settings);"

            string gpuFanSpeedLoad = "GFANL";  //added RSH 30/11/2020  see NvidiaGPU.cs "control = new Sensor("GPU Fan", 0, SensorType.Control, this, settings);"
            string gpuFanSpeedRPM = "GRPM";    //added RSH 30/11/2020  see NvidiaGPU.cs  "private readonly Sensor fan;"
            string gpuPower = "GPWR";          //added RSH 4/12/2020   see NvidiaGPU.cs  "power = new Sensor("GPU Power", 0, SensorType.Power, this, settings);"
            ...
            string cpuLoad = "";
            string ramLoad = "";
            string ramUsed = "";             // LibreHardwareMonitor and OpenHardwareMonitor used different Case definitions, see further down!!!!
            string ramAvailable = "";        // This is the RAM still available, not the total!!!
            string cpuName = "";
            string gpuName = "";
            string gpuCoreClock = "";
            ...
            */
            ...
            /*
          // Libre Specific
          string dataDownloaded = "";       // libre Network.cs _dataUploaded = new Sensor("Data Uploaded", 2, SensorType.Data, this, settings);
          string dataUploaded = "";         // libre Network.cs _dataDownloaded = new Sensor("Data Downloaded", 3, SensorType.Data, this, settings);
          string uploadSpeed = "";          // libre Network.cs _uploadSpeed = new Sensor("Upload Speed", 7, SensorType.Throughput, this, settings);
          string downloadSpeed = "";        // libre Network.cs  _downloadSpeed = new Sensor("Download Speed", 8, SensorType.Throughput, this, settings);
          */
            ...
            foreach (OpenHardwareMonitor.Hardware.IHardware hw in thisComputer.Hardware)// for each hardware item thisComputer
            {
                ...
                foreach (OpenHardwareMonitor.Hardware.ISensor s in hw.Sensors)          //for each sensor in the sensors part of the hardware
                {
                    ...
                    if (s.SensorType == OpenHardwareMonitor.Hardware.SensorType.SmallData)           // if the sensor is a data value etc.etc.
                    {
                        if (s.Value != null)
                        {
                            switch (s.Name)
                    ...
                    if (s.SensorType == OpenHardwareMonitor.Hardware.SensorType.Fan)           // if the sensor is a data value etc.etc.
                    {
                        if (s.Value != null)
                        {
                            switch (s.Name)
                    ...
                    if (s.SensorType == OpenHardwareMonitor.Hardware.SensorType.Control)           // if the sensor is a data value etc.etc.
                    {
                        if (s.Value != null)
                        {
                            switch (s.Name)
                    ...
                    if (s.SensorType == OpenHardwareMonitor.Hardware.SensorType.Power)           // if the sensor is a data value etc.etc.
                    {
                        if (s.Value != null)
                        {
                            switch (s.Name)
            ...
            // HardwareSerialMonitor v1.1 String
           ...
           // HardwareSerialMonitor v1.3 String
                            ...
                            if (sInstanceName.IndexOf(Vid_Pid) > -1) //if the string Vid_Pid is present in the string
                            {
                                if (isConnected == false) // if not already connected
                                {
                                    sPortName = queryObj["PortName"].ToString();// set the sPortName to the portname in the query
                                                                                // mySerialPort = new SerialPort(sPortName, 9600, Parity.None, 8, StopBits.One);// create a new port instance with values in the argument
                                    mySerialPort = new SerialPort(sPortName, 57600, Parity.None, 8, StopBits.One);// create a new port instance with values in the argument
                                    mySerialPort.ReadTimeout = 500;//set the timeouts
                                    mySerialPort.WriteTimeout = 500;
                                    mySerialPort.DtrEnable = true;
                                    mySerialPort.RtsEnable = true;
